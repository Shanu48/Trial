"""
Encrypt the message "Top Secret Data" using AES-192 with the
key "FEDCBA9876543210FEDCBA9876543210".
Show all the steps involved in the encryption process (key expansion, initial round, main rounds, final round).
"""

import sys
from copy import deepcopy
from Crypto.Util.Padding import pad

# === AES CONSTANTS (S-BOX and RCON) ===
s_box = [
    
]
  # Same as above, omitted for brevity
rcon = [
    0x00, 0x01, 0x02, 0x04, 0x08,
    0x10, 0x20, 0x40, 0x80, 0x1B,
    0x36, 0x6C, 0xD8, 0xAB, 0x4D,
    0x9A, 0x2F, 0x5E, 0xBC, 0x63,
    0xC6, 0x97, 0x35, 0x6A, 0xD4
]

def xtime(a):
    return ((a << 1) ^ 0x1B) & 0xFF if a & 0x80 else (a << 1)

def gmul(a, b):
    p = 0
    for _ in range(8):
        if b & 1:
            p ^= a
        a = xtime(a)
        b >>= 1
    return p

def bytes_to_matrix(b):
    return [list(b[i::4]) for i in range(4)]

def matrix_to_bytes(m):
    return bytes(sum(zip(*m), ()))

def sub_bytes(state):
    for i in range(4):
        for j in range(4):
            byte = state[i][j]
            state[i][j] = s_box[byte]
    print_state("After SubBytes", state)

def shift_rows(state):
    for i in range(4):
        state[i] = state[i][i:] + state[i][:i]
    print_state("After ShiftRows", state)

def mix_single_column(col):
    t = col[:]
    col[0] = gmul(t[0], 2) ^ gmul(t[1], 3) ^ t[2] ^ t[3]
    col[1] = t[0] ^ gmul(t[1], 2) ^ gmul(t[2], 3) ^ t[3]
    col[2] = t[0] ^ t[1] ^ gmul(t[2], 2) ^ gmul(t[3], 3)
    col[3] = gmul(t[0], 3) ^ t[1] ^ t[2] ^ gmul(t[3], 2)

def mix_columns(state):
    for i in range(4):
        col = [state[j][i] for j in range(4)]
        mix_single_column(col)
        for j in range(4):
            state[j][i] = col[j]
    print_state("After MixColumns", state)

def add_round_key(state, round_key):
    for i in range(4):
        for j in range(4):
            state[i][j] ^= round_key[i][j]
    print_state("After AddRoundKey", state)

def print_state(title, state):
    print(f"{title}:")
    for row in state:
        print(" ".join(f"{b:02X}" for b in row))
    print()

def key_expansion_192(key):
    nk = 6
    nb = 4
    nr = 12
    w = list(key)
    i = nk
    while len(w) < 4 * (nr + 1) * 4:
        temp = w[-4:]
        if i % nk == 0:
            temp = temp[1:] + temp[:1]  # Rotate
            temp = [s_box[b] for b in temp]
            temp[0] ^= rcon[i // nk]
        elif nk > 6 and i % nk == 4:
            temp = [s_box[b >> 4][b & 0x0F] for b in temp]
        w += [w[-nk*4 + j] ^ temp[j] for j in range(4)]
        i += 1
    round_keys = [bytes_to_matrix(bytes(w[16*i:16*(i+1)])) for i in range(nr + 1)]
    return round_keys

# === User Input ===
plaintext = input("Enter plaintext: ")
key_input = input("Enter 192-bit key (48 hex characters): ")
if len(key_input) != 48:
    print("❌ Invalid key length. Must be 48 hex characters (192 bits).")
    sys.exit(1)

key = bytes.fromhex(key_input)
padded = pad(plaintext.encode(), 16)[:16]  # Use first block
state = bytes_to_matrix(padded)
print_state("Initial State (plaintext)", state)

round_keys = key_expansion_192(key)

# === Initial Round ===
add_round_key(state, round_keys[0])

# === Main Rounds (1 to 11) ===
for rnd in range(1, 12):
    print(f"--- Round {rnd} ---")
    sub_bytes(state)
    shift_rows(state)
    mix_columns(state)
    add_round_key(state, round_keys[rnd])

# === Final Round ===
print("--- Final Round (12) ---")
sub_bytes(state)
shift_rows(state)
add_round_key(state, round_keys[12])

print_state("Final State (Ciphertext)", state)
ciphertext = matrix_to_bytes(state)
print(f"\nFinal Ciphertext (hex): {ciphertext.hex().upper()}")
